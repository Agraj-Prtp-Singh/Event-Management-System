...
function getInitialSettings(settingsStorageKey, fallbackName) {
  const savedSettings = localStorage.getItem(settingsStorageKey);
  const user = getStoredUser() || {};
  const parsedSettings = savedSettings ? JSON.parse(savedSettings) : {};

  return {
    fullName: parsedSettings.fullName || user.fullName || fallbackName,
...
export default function AccountSettings({
  ...
  const [savedSettings, setSavedSettings] = useState(() =>
    getInitialSettings(settingsStorageKey, fallbackName),
  );
  ...
  const handleProfileSave = (event) => {
    ...
    localStorage.setItem(settingsStorageKey, JSON.stringify(profileData));
